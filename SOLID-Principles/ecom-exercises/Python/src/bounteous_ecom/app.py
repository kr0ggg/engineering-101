...
class EcommerceManager:
    ...
    def process_order(self, customer_id: int):
        ...
        order.order_number = f"ORD-{datetime.now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:8].upper()}"
        order.items = []
        order.subtotal = self.calculate_cart_total(customer_id)
        order.tax_amount = self.calculate_cart_total(customer_id) * Decimal('0.08')  # 8% tax
        order.shipping_amount = Decimal('9.99')
        order.total_amount = self.calculate_cart_total(customer_id) + (self.calculate_cart_total(customer_id) * Decimal('0.08')) + Decimal('9.99')
