...
export class EcommerceManager {
    ...
    async addToCart(customerId: number, productId: number, quantity: number): Promise<void> {
        ...
        if (existingItemRes.rows.length > 0) {
            ...
            await this.client.query(
                ...
                [cart.id, productId, quantity, product.price, totalPrice]
            );
        }

        console.log(`Added ${quantity} x ${product.name} to cart for customer ${customerId}`);
    }
    ...
    async processOrder(customerId: number): Promise<void> {
        ...
        await this.client.query('DELETE FROM carts WHERE id = $1', [cart.id]);
    ...
    async generateInvoice(orderId: number): Promise<void> {
        ...
        const invoiceRes = await this.client.query(
            'INSERT INTO invoices (order_id, invoice_number, amount, tax_amount, total_amount, invoice_date) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
            [orderId, invoice.invoiceNumber, invoice.amount, invoice.taxAmount, invoice.totalAmount, invoice.invoiceDate]
        );
    ...
    async generateSalesReport(): Promise<void> {
        ...
        const totalSales = ordersRes.rows[0].total || 0;
